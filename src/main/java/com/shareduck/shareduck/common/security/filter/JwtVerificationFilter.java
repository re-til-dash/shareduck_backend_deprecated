...
public class JwtVerificationFilter extends OncePerRequestFilter {
    ...
    private static final String EXCEPTION = "exceptionCode";
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            request.setAttribute(EXCEPTION, AuthExceptionCode.ACCESS_TOKEN_EXPIRED);
        } catch (SignatureException se) {
            request.setAttribute(EXCEPTION, AuthExceptionCode.INVALID_SIGNATURE_ACCESS_TOKEN);
        } catch (JwtException e) {
            log.warn("Exception : {}, message : {}", e.getClass(), e.getMessage());
            request.setAttribute(EXCEPTION, AuthExceptionCode.UNAUTHENTICATED);
        }
